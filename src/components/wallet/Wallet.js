...
export default class Wallet extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      wallet: {},
      import_key: "",
      archive_active: false, //transaction
      send_coin: "safex",
      send_amount: 1,
      send_fee: 0.0001,
      send_total: 1,
      ...
      history_key: "",
      send_to: "",
      send_keys: { public_key: "", private_key: "" },
      ...
      collapse_open: { key: "", receive_open: false, send_open: false },
      private_key_open: {
        key: "",
        display_private_key: "",
  ...
  componentWillUnmount() {
    this.setState({
      ...
      sidebar_open: false, // Close Settings Modal
      settings_active: false,
      // Close Send Receive Modal
      collapse_open: { send_open: false, receive_open: false }
    });
  ...
  componentDidMount = () => {
    ...
    this.getFee();
    this.getPrices();
    this.setRefreshInterval = setInterval(this.refreshWalletInterval, 600000);
  };
  ...
  prepareDisplayPendingTx = (keys = null) => {
    ...
    Promise.all(promises).then(values => {
      ...
      for (var x = 0; x < values.length; x++) {
        var currentIndex = this.state.keys.findIndex(
          i => i.public_key === keys[iteration]["public_key"]
        );
  ...
  prepareDisplay = (keys = null) => {
    ...
    Promise.all(promises)
      .then(values => {
        ...
        for (var x = 0; x < values.length; x++) {
          var currentIndex = this.state.keys.findIndex(
            i => i.public_key === keys[internal_index]["public_key"]
          );
          if (iteration === 0) {
            this.state.keys[currentIndex].safex_bal = values[x].balance;
            iteration += 1;
          } else if (iteration === 1) {
            this.state.keys[currentIndex].btc_bal = (
              values[x] / 100000000
            ).toFixed(8);
            iteration += 1;
          } else if (iteration === 2) {
            this.state.keys[currentIndex].pending_btc_bal = (
              values[x] / 100000000
            ).toFixed(8);
            iteration = 0;
            internal_index += 1;
  ...
  formBitcoinTransaction = (utxos, amount, fee, destination, key, source) => {
    ...
    fetch("http://omni.safex.io:3001/broadcast", {
      method: "POST",
      body: JSON.stringify(json)
    })
      .then(resp => resp.text())
  ...
  formSafexTransaction = (utxos, amount, fee, destination, key, source) => {
    ...
    fetch("http://omni.safex.io:3001/getsafextxn", {
      method: "POST",
      body: JSON.stringify(SafexTransaction)
    })
      .then(resp => resp.text())
      .then(resp => {
        ...
        checks.forEach(function(item) {
          txn.__tx.outs.some(out => {
            try {
              var pubkey = bitcoin.address.fromOutputScript(
                out.script,
                bitcoin.networks.livenet
              );
            } catch (e) {
              console.log(e);
            }
    ...
    if (this.state.send_coin === "safex") {
      if (this.state.send_fee < 0.00001) {
        if (this.sidebar_open) {
          ...
          this.setCoinModalOpenSettings(
            "The fee is too small. Minimum fee is 0.00001000 BTC"
          );
        } else {
          this.setState({
            send_fee: parseFloat(0.00001).toFixed(8)
          });
          this.setCoinModalClosedSettings(
            "The fee is too small. Minimum fee is 0.00001000 BTC"
          );
        }
      } else if (
        (this.state.send_coin === "safex" &&
          this.state.send_fee > key_btc_bal) ||
        (this.state.send_coin === "btc" && this.state.send_total > key_btc_bal)
      ) {
        if (this.sidebar_open) {
          this.setCoinModalOpenSettings(
            "You do not have enough BITCOIN to cover the fee"
          );
        } else {
          this.setCoinModalClosedSettings(
            "You do not have enough BITCOIN to cover the fee"
        ...
        this.state.send_coin === "safex" &&
        this.state.send_amount > key_safex_bal
      ) {
        if (this.sidebar_open) {
          this.setCoinModalOpenSettings(
            "You do not have enough SAFEX to cover this transaction"
          );
        } else {
          this.setCoinModalClosedSettings(
            "You do not have enough SAFEX to cover this transaction"
        ...
        try {
          ...
          try {
            fetch(
              "http://bitcoin.safex.io:3001/insight-api/addr/" +
                e.target.public_key.value +
                "/utxo"
            )
              .then(resp => resp.json())
              .then(resp => {
                //enter into the safex transaction method
                this.formSafexTransaction(
                  resp,
                  amount,
      ...
      if (this.state.send_fee < 0.00001) {
        ...
        this.state.send_coin === "btc" &&
        this.state.send_total > key_btc_bal
      ) {
        if (this.sidebar_open) {
          this.setCoinModalOpenSettings(
            "You do not have enough BITCOIN to cover this transaction"
          );
        } else {
          this.setCoinModalClosedSettings(
            "You do not have enough BITCOIN to cover this transaction"
  ...
  openCoinModal = e => {
    ...
    if (
      (this.state.send_coin === "safex" && this.state.send_fee > key_btc_bal) ||
      (this.state.send_coin === "btc" && this.state.send_total > key_btc_bal)
    ) {
      ...
      (this.state.send_amount === "" ||
        this.state.send_amount === 0 ||
        isNaN(this.state.send_amount))
    ) {
      if (this.state.sidebar_open) {
      ...
      this.state.send_coin === "btc" &&
      (this.state.send_amount === "" ||
        parseFloat(this.state.send_amount) === 0 ||
        isNaN(this.state.send_total))
    ) {
      if (this.state.sidebar_open) {
      ...
      if (this.state.sidebar_open) {
        this.setCoinModalOpenSettings(
          "Cannot send transaction, this key has a pending minus. Please try again later."
        );
      } else {
        this.setCoinModalClosedSettings(
          "Cannot send transaction, this key has a pending minus. Please try again later."
      ...
      if (this.state.sidebar_open) {
        this.setCoinModalOpenSettings(
          "Please convert the transaction to BTC to proceed"
        );
      } else {
        this.setCoinModalClosedSettings(
          "Please convert the transaction to BTC to proceed"
    ...
    fs.writeFile(localStorage.getItem("wallet_path"), cipher_text, err => {
      if (err) {
        ...
        try {
          ...
          this.openMainAlertPopup(
            "An error adding a key to the wallet. Please contact team@safex.io"
          );
        }
      }
    });
  ...
  importKeyChange = e => {
    this.setState({ import_key: e.target.value });
  };
    ...
    try {
      ...
      try {
        ...
        if (key_exists === false) {
          ...
          fs.writeFile(
            localStorage.getItem("wallet_path"),
            cipher_text,
            err => {
              if (err) {
                console.log(err);
      ...
      this.setState({
        // Close Private Key Popup
        private_key_open: { private_key_popup: false }
      });
      this.openMainAlertPopup("Invalid private key");
    }
    this.setState({
  ...
  importGlow = () => {
    this.setState({ import_wrap_glow: true });
  };

  importGlowDeactivate = () => {
    this.setState({ import_wrap_glow: false });
  };
  ...
  exportUnencryptedWallet = () => {
    ...
    this.setState({ main_alert_popup: false });
    ...
    if (sendreceive === "send") {
      if (
        (!this.state.collapse_open.send_open &&
          this.state.collapse_open.key !== key) ||
        (this.state.collapse_open.send_open &&
          this.state.collapse_open.key !== key)
      ) {
        this.setState({
          ...
          (err, wallet) => {
            if (!err) {
              ...
              if (hash1.toString() === hash2.toString()) {
                ...
                const encrypted_wallet = encrypt(
                  decrypted_wallet_str,
                  algorithm,
                  password
                );
                  ...
                  err => {
                    if (err) {
                      this.setState({
                        info_popup: true,
                        info_text:
                          "There was a problem writing the new encrypted file to disk"
                      });
                    } else {
                      ...
                      this.setState({
                        info_popup: true,
                        info_text:
                          "Password has been changed. Write it down, keep it safe."
                      });
                    }
                  }
                );
  ...
  closeSettingsInfoPopup = () => {
    this.setState({ info_popup: false });
  };
      ...
      get_utxos(this.state.send_public_key)
        .then(utxos => {
          ...
          this.setState({
            send_amount: 1,
            send_total: 1,
            send_fee: (parseFloat(rawtx.fee / 100000000) + 0.000007).toFixed(8)
          });
        })
        .catch(err => {
          console.log("generate safex transaction error" + err);
          this.openMainAlertPopup(
            "Safex Transaction Error; You may have a 0 balance \n" + err
          );
        })
        .catch(err => {
          console.log("error getting UTXOs " + err);
          this.openMainAlertPopup(
            "error getting UTXOs; You may have a connectivity issue " + err
          );
        });
    } else {
      get_utxos(this.state.send_public_key)
        .then(utxos => {
          ...
          this.setState({
            ...
            send_total: parseFloat(
              parseFloat(0.00000001) + parseFloat(rawtx.fee / 100000000)
            ).toFixed(8)
          });
        })
        .catch(err => {
          console.log(
            "generate btc transaction error you might have no money " + err
          );
          this.openMainAlertPopup(
            "BTC Transaction Error; You may have a 0 balance \n" + err
          );
        })
        .catch(err => {
          console.log("error getting UTXOs " + err);
  ...
  listTransactions(key) {
    ...
    promises.push(
      fetch(
        `http://bitcoin.safex.io:3001/insight-api/txs/?address=${key}`
      ).then(res => {
        return res.json();
      })
    );
    ...
    Promise.all(promises)
      .then(function(response) {
        ...
        var array = historyBtc.map(tx => ({
          ...
          date_time:
            tx.propertyname === "SafeExchangeCoin" ? tx.blocktime : tx.time,
          safex_txid: tx.txid,
        ...
        JSON.parse(txArray).forEach(tx => {
          ...
          if (
            btc_sending_direction[0] !== undefined &&
            btc_sending_direction[0].length > 0
          ) {
          ...
          if (safex_direction === "Received" && coin === "safex") {
            render +=
              ...
              tx["safex_txid"] +
              `</p><br />
                            <p class="address address-green">` +
              tx["safex_sending_address"] +
              `</p> <p class="address-arrow"> ➡ </p> <p class="address address-green">` +
              tx["safex_reference_address"] +
              `</p>
        ...
        if (err) {
          ...
          try {
            ...
            json2["keys"].forEach(key => {
              var currentIndex = this.state.keys.findIndex(
                i => i.public_key === key["public_key"]
              );
              key["safex_bal"] = this.state.keys[currentIndex].safex_bal;
              key["btc_bal"] = this.state.keys[currentIndex].btc_bal;
              key["pending_safex_bal"] = this.state.keys[
                currentIndex
              ].pending_safex_bal;
              key["pending_btc_bal"] = this.state.keys[
                currentIndex
              ].pending_btc_bal;
            });
  ...
  removeFromArchive = index => {
      ...
      fs.writeFile(localStorage.getItem("wallet_path"), cipher_text, err => {
        if (err) {
          ...
          try {
            ...
            this.openMainAlertPopup(
              "An error occured while adding a key to the wallet. Please contact team@safex.io"
            );
          }
        }
      });
    ...
    this.setState({ transfer_key_to_home: true });
    setTimeout(() => {
      this.setState({
        transfer_key_to_home: false
      });
    ...
    if (e.target.name === "total_trade_volume") {
      ...
      safexDividendYield =
        (((parseFloat(this.state.marketplaceFee) / 100) *
          ...
          2147483647) *
        parseFloat(e.target.value) *
        (100 / parseFloat(this.state.holdingsByMarket));
      this.setState({
  ...
  saveLabel = e => {
    this.setState({ savedLabel: e.target.value });
  };
  ...
  render() {
    ...
    var table = Object.keys(keys).map(key => {
      return (
        <div
          className={
            keys[key].archived === archive_active ||
            (!keys[key].hasOwnProperty("archived") && archive_active === false)
              ? "col-xs-12 single-key"
              : "col-xs-12 single-key hidden-xs hidden-sm hidden-md hidden-lg"
          }
          key={key}
        >
          <div className="col-xs-8 single-key-inner">
            ...
            <div className="key">
              <form onSubmit={this.copyToClipboard}>
                {this.state.copied &&
                keys[key].public_key === this.state.current_public_key ? (
                  <CopyToClipboard
                    text={keys[key].public_key}
                ...
                <input
                  type="hidden"
                  name="current_public_key"
                  id="current_public_key"
                  readOnly
                  value={keys[key].public_key}
                />
              </form>
          ...
          <div className="pull-right single-key-btns-wrap">
            ...
            this.state.collapse_open.key === key ? (
              <div>
                <button
                  disabled={
                    (keys[key].pending_btc_bal >= 0 &&
                      this.state.average_fee !== 0) ||
                    this.state.send_disabled === false
                      ? ""
                      : "disabled"
                  }
                  ...
                  className={
                    ...
                    this.state.send_disabled
                      ? "send-btn button-shine disabled-btn"
                      : "send-btn button-shine active"
                  }
                >
                ...
                <button
                  className="receive-btn button-shine disabled"
              ...
              <div>
                <button
                  ...
                  className={
                    (this.state.collapse_open.key === key &&
                      this.state.collapse_open.receive_open) ||
                    this.state.send_disabled
                      ? "send-btn button-shine disabled"
                      : "send-btn button-shine"
                  }
                >
                  {(this.state.collapse_open.key === key &&
                    this.state.collapse_open.receive_open) ||
                ...
                <button
                  className="receive-btn button-shine-green"
                  onClick={this.openSendReceive.bind(this, key, "receive")}
                >
                  {this.state.collapse_open.key === key &&
                  this.state.collapse_open.receive_open ? (
            ...
            <div className="row amounts">
              <div className="row amounts">
                <div className="col-xs-5 amount-btns-wrap">
                  <button
                    onClick={() => this.removeFromArchive(key)}
                    className={
                      keys[key].archived === true ||
                      (!keys[key].hasOwnProperty("archived") &&
                        archive_active === true)
                  ...
                  <button
                    onClick={() => this.sendToArchive(key)}
                    className={
                      keys[key].archived === false ||
                      (!keys[key].hasOwnProperty("archived") &&
                  ...
                  {this.state.history_overflow_active ? (
                    <button
                      onClick={this.setCloseHistoryModal}
                    ...
                    <button
                      onClick={() => this.setOpenHistoryModal(key)}
                      className="archive-button history-button button-shine history-btn"
                    >
                      <span>HISTORY</span>
                    </button>
                  ...
                  <button
                    onClick={() => this.showPrivateModal(key)}
                ...
                <div className="amounts safex-amounts">
                  <span className="col-xs-12 amount">
                    <span>{parseFloat(keys[key].safex_bal)}</span>
                ...
                <div className="amounts bitcoin-amounts">
                  <span className="col-xs-12 amount">
                    <span>
                      {keys[key].pending_btc_bal < 0
                        ? (
                            parseFloat(keys[key].btc_bal) +
                            parseFloat(keys[key].pending_btc_bal)
                          ).toFixed(8)
                        : keys[key].btc_bal}
                    </span>
                    <span className="coin-name">Bitcoin</span>
                    <span>${(keys[key].btc_bal * btc_price).toFixed(2)}</span>
                  </span>
                </div>
              </div>
            </div>
          ...
          >
            <div
              className="col-xs-12 sendCloseButton button-shine"
              onClick={this.openSendReceive.bind(this, key, "send")}
            >
              <div className="close text-right">X</div>
            </div>
            <div className="row">
              <div className="col-xs-7 send-left">
                ...
                <img
                  className={
                    this.state.send_coin === "safex" ? "coin active" : "coin"
                  }
                  onClick={this.sendCoinChoose.bind(this, "safex")}
                  src="images/coin-white.png"
                  alt="Safex Coin"
                />
                <img
                  className={
                    this.state.send_coin === "btc" ? "coin active" : "coin"
                  }
                  onClick={this.sendCoinChoose.bind(this, "btc")}
                ...
                <input
                  ...
                  name="which"
                  readOnly
                  value={this.state.send_coin}
                />
                <input
                  type="hidden"
                  name="private_key"
                  readOnly
                  value={keys[key].private_key}
                />
                <input
                  type="hidden"
                  name="public_key"
                ...
                <input
                  type="hidden"
                  name="pending_safex_bal"
                  readOnly
                  value={keys[key].pending_safex_bal}
                  id="pending_safex_bal"
                />
                <input
                  type="hidden"
                  name="pending_btc_bal"
                  readOnly
                  value={keys[key].pending_btc_bal}
                  id="pending_btc_bal"
                />
                ...
                <div className="input-group">
                  ...
                  <input
                    ...
                    onChange={this.setCloseSendReceivePopup}
                  />
                </div>
              </div>
              <div className="col-xs-5 send-right">
                  ...
                  {this.state.send_coin === "safex" ? (
                    <input
                      ...
                      value={
                        this.state.fee_in_$
                          ? (
                              this.state.send_amount * this.state.safex_price
                            ).toFixed(8)
                          : this.state.send_amount
                      }
                    />
                    ...
                    <input
                      ...
                      value={
                        this.state.fee_in_$
                          ? (
                              this.state.send_amount * this.state.btc_price
                            ).toFixed(8)
                          : this.state.send_amount
                      }
                    />
                ...
                <div className="form-group">
                  <label htmlFor="fee">
                    {this.state.fee_in_$ ? "Fee ($)" : "Fee (BTC):"}
                  </label>
                  <input
                    type="number"
                    name="fee"
                    onChange={this.sendFeeOnChange}
                    readOnly={this.state.fee_in_$ ? "readonly" : ""}
                    value={
                      this.state.fee_in_$
                        ? (this.state.send_fee * this.state.btc_price).toFixed(
                            8
                          )
                        : this.state.send_fee
                    }
                  ...
                  {this.state.send_coin === "safex" ? (
                    <input
                      ...
                      value={
                        this.state.fee_in_$
                          ? (
                              this.state.send_total * this.state.safex_price
                            ).toFixed(8)
                          : this.state.send_total
                      }
                    />
                  ) : (
                    <input
                      ...
                      value={
                        this.state.fee_in_$
                          ? (
                              this.state.send_total * this.state.btc_price
                            ).toFixed(8)
                          : this.state.send_total
                      }
                    />
                ...
                <div className="form-group">
                  {this.state.send_coin === "safex" ? (
                    ...
                    <button
                      type="button"
                      className="btc-convert-btn button-shine"
                      onClick={this.convertBtcToDollars}
                      disabled={
                        this.state.send_overflow_active &&
                        this.state.transaction_sent === false
                          ? "disabled"
                          : ""
                      }
                    >
                  ...
                  this.state.transaction_sent === false ? (
                    <button
                      ...
                      disabled={
                        this.state.transaction_being_sent ? "disabled" : ""
                      }
                    >
          ...
          <div
            className={
              this.state.private_key_open.private_key_popup &&
              keys[key].public_key ===
                this.state.private_key_open.current_public_key
                ? "col-xs-12 private_key_popup active"
                : "col-xs-12 private_key_popup"
            }
          >
            <h4>
              The following key is to control your coins, do not share it. Keep
              your private key for yourself only!
            </h4>
            <p>{this.state.private_key_open.display_private_key}</p>
            <button
              className="button-shine"
              onClick={this.setClosePrivateModal}
            >
              Ok
            </button>
          </div>
    ...
    return (
      <div
        className={
          this.state.loging_out ? "wallet-page fadeOutUp" : "wallet-page"
        }
      >
        <Navigation
          wallet={this.wallet}
        ...
        <div className="container keys-container fadeIn">
          ...
          >
            <div
              className={
                this.state.history_overflow_active ? "row row-hidden" : "row"
              }
            >
              {table}
            </div>
          ...
          <div
            className={
              this.state.send_overflow_active
                ? "keys-backdrop active"
                : "keys-backdrop"
            }
            onClick={
              this.state.send_overflow_active ? this.setCloseCoinModal : ""
            }
          />
        </div>
        ...
        >
          ...
          <SettingsModal
            ...
            openExportUnencryptedWalletPopup={
              this.setOpenExportUnencryptedWalletPopup
            }
            logout={this.logout}
          />
