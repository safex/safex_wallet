...
export default class Migrate2 extends React.Component {
  ...
  setYourKeys = e => {
    ...
    if (
      ...
      for (key in this.state.safex_keys) {
        console.log(this.state.safex_keys[key].spend.sec);
        if (
          this.state.safex_keys[key].spend.sec === e.target.spend_key.value &&
      ...
      if (
        ...
        const safex_keys = structureSafexKeys(
          e.target.spend_key.value,
          e.target.view_key.value
        );
      ...
      for (var key in json.keys) {
        if (json.keys[key].public_key === this.props.data.address) {
          ...
          for (var x_key in json.safex_keys) {
            if (
              json.safex_keys[x_key].public_addr ===
              e.target.address_selection.value
            ) {
              x_index = x_key;
              json.keys[key_index]["migration_data"] = {};
              json.keys[key_index]["migration_data"].safex_keys =
                json.safex_keys[x_index];
  ...
  render() {
    ...
    if (this.state.safex_keys) {
      Object.keys(this.state.safex_keys).map(key => {
        options.push(
          <option key={key} value={this.state.safex_keys[key].public_addr}>
            {this.state.safex_keys[key].public_addr}
          </option>
        );
      });
        ...
        {this.state.create_address ? (
          <div>
            ...
            <p className="red-text">
              The following wallet information is to control your coins, do not
              share it. <br />
              Sharing this information can and will result in total loss of your
              Safex Tokens and Safex Cash.
              <br />
            ...
            <input
              ...
              defaultValue={this.state.safex_address}
            />
            ...
            <button className="button-shine" onClick={this.startOver}>
              Go back
            </button>
            <button
              className="button-shine green-btn"
              onClick={this.saveSafexKeys}
            >
              Back up my Safex Keys and continue
            </button>
          </div>
        ) : (
          <div>
            {this.state.used_addresses ? (
              <div>
                ...
                <form
                  className="previuously-used-form"
                  onSubmit={this.selectKey}
                >
                  <select name="address_selection">{options}</select>
                    ...
                    <div className="set-your-key-head">
                      ...
                      <div className="set-your-key-right">
                        <h3 className="blue-text">
                          Migration using my existing Safex address
                        </h3>
                      </div>
                    </div>
                    ...
                    <form onSubmit={this.setYourKeys}>
                      <div className="form-group">
                        ...
                        <label
                          ...
                          htmlFor="safex_address"
                        >
                          If you already have Safex address, enter it here
                        </label>
                      </div>
                      <div className="form-group">
                        <input
                        ...
                        <label
                          ...
                          htmlFor="spend_key"
                        >
                          Enter your Safex address secret spend key here
                        </label>
                      </div>
                      <div className="form-group">
                        <input
                        ...
                        <label
                          ...
                          htmlFor="view_key"
                        >
                          Enter your Safex address secret view key here
                        </label>
                      </div>

                      <button
                        className={
                          this.state.all_field_filled
                            ? "button-shine green-btn"
                            : "button-shine"
                        }
                      >
                        Set your address
