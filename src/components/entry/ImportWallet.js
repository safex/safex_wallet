...
export default class ImportWallet extends React.Component {
  ...
  handleSubmit(e) {
    ...
    if (!targetPassword) {
      if (this.state.walletImportAlerts === false) {
        this.openWalletImportAlert(
          `You must enter password for the wallet file`,
          5000
        );
      }
      this.wrongTargetPassword();
    ...
    if (this.state.currentEncryptedWallet) {
      currentPassword =
        e.target.current_password && e.target.current_password.value;
      if (!currentPassword) {
        if (this.state.walletImportAlerts === false) {
          this.openWalletImportAlert(
            `You must enter password for your current wallet. 
                    If you want to throw it away and replace it with this new one, go back and click "RESET WALLET" in the top right corner first.`,
            12000
          );
        }
    ...
    return loadAndDecryptWalletFromFile(
      this.state.path,
      targetPassword,
      (err, targetWallet) => {
        if (!err && !targetWallet) {
        ...
        if (err) {
          ...
          if (this.state.walletImportAlerts === false) {
            this.openWalletImportAlert(
              "Failed to load target wallet: " + err.message,
              5000
            );
          }
          this.wrongTargetPassword();
          ...
          return fs.writeFile(
            ...
            err => {
              ...
              localStorage.setItem(
                "wallet",
                JSON.stringify(targetWallet.decrypted)
              );
        ...
        let decrypted;
        try {
          decrypted = decryptWalletData(
            this.state.currentEncryptedWallet,
            currentPassword
          );
        } catch (err) {
          if (this.state.walletImportAlerts === false) {
            this.openWalletImportAlert(
              "Failed to access your current wallet: " + err.message,
              5000
            );
          }
          this.wrongCurrentPassword();
        ...
        return fs.writeFile(walletPath, reEncrypted, err => {
          ...
          const duplicatesMessage =
            importedCount < targetKeys.length
              ? ` (found ${targetKeys.length -
                  importedCount} BTC Key duplicates)`
              : "";
          const duplicatesSafexMessage =
            importedSafexCount < targetSafexKeys.length
              ? ` (found ${targetSafexKeys.length -
                  importedSafexCount} Safex Key duplicates)`
              : "";
          if (this.state.walletImportAlerts === false) {
            this.openWalletImportAlert(
              `Imported ${importedCount} out of ${
                targetKeys.length
              } keys ${duplicatesMessage}.
                     Import ${importedSafexCount} out of ${
                targetSafexKeys.length
              } keys ${duplicatesSafexMessage}`,
              4000
            );
          }
          ...
          <form className="form-group" onSubmit={this.handleSubmit}>
            ...
            <div className="col-xs-12 fileandpass">
              ...
              <input
                type="password"
                className={
                  this.state.wrongTargetPassword
                    ? "form-control shake"
                    : "form-control"
                }
                name="password"
            ...
            <button className="btn btn-default button-neon-green" type="submit">
              IMPORT
            </button>
          </form>
          <p className="text-center">Write password down and NEVER lose it.</p>
        </div>
