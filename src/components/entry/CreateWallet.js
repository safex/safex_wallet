...
export default class CreateWallet extends React.Component {
  ...
  handleSubmit(e) {
    ...
    if (
      e.target.create_password.value.length > 0 &&
      e.target.create_password.value === e.target.repeat_password.value
    ) {
      ...
      fs.writeFile(walletPath, cipher_text, err => {
        if (err) {
          ...
          this.openWalletImportAlert(
            `Failed to write wallet to ${walletPath}: ${err.message}`,
            5000
          );
        } else {
    ...
    if (
      (e.target.create_password.value.length > 0 &&
        e.target.repeat_password.value.length === 0) ||
      (e.target.repeat_password.value.length > 0 &&
        e.target.repeat_password.value !== e.target.create_password.value)
    ) {
      ...
      <div className="container">
        <div className="col-xs-12 Login-logo">
          ...
          <Link className="back-button" to="/">
            <img src="images/back.png" alt="Back img" /> Back
          </Link>
        </div>
        <div className="col-xs-12 Login-form Create-wallet-form">
          <form className="form-group" onSubmit={this.handleSubmit}>
            ...
            <input
              className={
                this.state.wrongRepeatPassword
                  ? "form-control shake"
                  : "form-control"
              }
              type="password"
